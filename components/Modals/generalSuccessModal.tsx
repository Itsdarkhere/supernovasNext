...
const GeneralSuccessModal = () => {
  return (
    <>
      ...
      {/* *ngIf="this.buttonClickedAction === 'profileRoute' || this.buttonClickedAction === 'general'" */}
      <div className="general-success-modal-container">
        ...
        {/* (click)="generalSuccessModalButtonClicked()" */}
        <button className="general-success-modal-button">{buttonText}</button>
      </div>
      {/* <!-- noProfile --> */}
      {/* *ngIf="this.buttonClickedAction === 'noProfile'" */}
      <div className="general-success-modal-container">
        <Image
          src={noProfileIcon}
          alt="No Profile Icon"
        ...
        {/* (click)="createProfileButtonClicked()" */}
        <button className="no-profile-button-1">Create Profile</button>
        {/* (click)="logInButtonClicked()" */}
        <button className="no-profile-button-2">Log In</button>
      </div>
      {/* <!-- connectWalletMobileError --> */}
      {/* *ngIf="this.buttonClickedAction === 'connectWalletMobileError'" */}
      <div className="general-success-modal-container">
        <p className="general-success-modal-header">{header}</p>
        <p className="general-success-modal-text-wallet-mobile-error">{text}</p>
        {/* (click)="generalSuccessModalButtonClicked()" */}
        <button className="general-success-modal-button">{buttonText}</button>
      </div>
    ...
    this.buttonClickedAction === 'ethMarketplaceError'
  " */}
      <div className="general-success-modal-container">
        <p className="general-success-modal-header">{header}</p>
        <p className="general-success-modal-text-wallet-mobile-error">{text}</p>
      ...
      {/* *ngIf="
    this.buttonClickedAction === 'connectWallet' &&
    !this.globalVars.imxWalletConnected &&
  ...
  " */}
      <div className="general-success-modal-container-connectedWallet">
        ...
        <p className="general-success-modal-text-connectedWallet">{{ text }}</p>
        {/* (click)="generalSuccessModalButtonClicked()" */}
        <button className="general-success-modal-button-logo imx-connect-button">
          <Image
            className="eth-logo h-100 d-flex flex-row flex-center ethlogo-white"
            src={ethIcon}
            alt="eth-logo"
          />
          <div className="general-success-modal-button-text">{buttonText}</div>
        </button>
      </div>
...
*ngIf="
  ...
  (clickOutside)="clickOutside()"*/}
      <div className="general-success-modal-container-connectedWallet-success">
        ...
        {/* (click)="wantToDepositButtonClicked()" */}
        <button className="general-success-modal-button-logo want-to-deposit-button">
          <div className="general-success-modal-button-text-connected">
            Deposit ETH to Immutable X
          </div>
        </button>
        {/* (click)="wantToBuyEthButtonClicked()" */}
        <button className="general-success-modal-button-logo want-to-buy-button">
          <div className="general-success-modal-button-text-connected">
            Buy ETH
          </div>
        </button>
