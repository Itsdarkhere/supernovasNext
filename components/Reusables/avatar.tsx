...
import { GetDefaultProfilePictureURL, GetSingleProfilePictureURL } from "../../utils/backendapi-context";
...
const Avatar = ({ avatar, classN }) => {
    ...
    const profileUpdateTimestamp = useAppSelector((state) => state.loggedIn.profileUpdateTimestamp);
    // Functions
    const setAvatar = () => {
        ...
        let fallbackRoute = `fallback=${GetDefaultProfilePictureURL(window.location.host)}`;
    ...
    return (
        <div ref={avatarRef} className={classN}></div>
    )
}
